...
/// 
/// Features:
/// - View details in a read-only format.
/// - Edit details in an inline-edit mode.
/// - Save updated data back to Firestore.
/// - Handle medication types (Eye/Non-Eye) with specific UI changes.
/// - Select application sites, frequency, schedule, and prescription dates.
...
class _MedicationDetailScreenState extends State<MedicationDetailScreen> {
  ...
  bool isLoading = true; // Flag to track loading state.
   ...
   Map<String, dynamic> editableMedication = {}; // Stores editable medication data.
  ...
  ///
  /// - Retrieves data from Firestore using `FirestoreService`.
  /// - Determines the correct collection based on medication type (Eye/Non-Eye).
  /// - Updates `editableMedication` with fetched data.
  /// - Handles errors and stops loading on failure.
  Future<void> _fetchMedicationData() async {
    try {
      ...
      String uid = user.uid;
      String collectionPath = widget.medication["medType"] == "Eye Medication"
      ...
      setState(() {
        ...
        isLoading = false; // Stops loading once data is fetched.
      });

    } catch (e) {
      log("Error fetching medication: $e");
      setState(() {
        isLoading = false; // If error occurs, stops loading.
      });
    }
  }
  ...
  /// - Handles conditional fields like `isIndefinite` duration and `applicationSite`.
  /// - Updates the Firestore document using `FirestoreService`.
  ...
  if (_formKey.currentState!.validate()) {
    ...
    if (editableMedication["medType"] != "Eye Medication") {
  ...
  /// Builds the UI for displaying or editing medication details
  @override
  Widget build(BuildContext context) {
    bool isEyeMedication = editableMedication["medType"] == "Eye Medication";
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (isEditing)
                            ...
                            children: [
                              Expanded(
                                ...
                                decoration: BoxDecoration(
                                  color: editableMedication["medType"] == "Eye Medication" ? Colors.blue : Colors.grey[300],
                                ...
                                child: Center(
                                  child: Text(
                                    "Eye Medication",
                                    style: TextStyle(
                                      color: editableMedication["medType"] == "Eye Medication" ? Colors.white : Colors.black,
                              ...
                              Expanded(
                                child: GestureDetector(
                                  onTap: () {
                                    setState(() {
                                      editableMedication["medType"] = "Non-Eye Medication";
                                  ...
                                  child: Container(
                                    ...
                                    margin: EdgeInsets.all(4),
                                    decoration: BoxDecoration(
                                      color: editableMedication["medType"] == "Non-Eye Medication" ? Colors.blue : Colors.grey[300],
                                      borderRadius: BorderRadius.circular(10),
                                    ),
                                    child: Center(
                                      child: Text(
                                        "Non-Eye Medication",
                                        style: TextStyle(
                                          color: editableMedication["medType"] == "Non-Eye Medication" ? Colors.white : Colors.black,
                                          fontWeight: FontWeight.bold,
                                        ),
                                      ),
                                    ),
                          ...
                          _buildDetailRow("Type", editableMedication["medType"] == "Eye Medication" ? 
                          "Eye Medication" : "Non-Eye Medication"),

                        // Show or disable Application Site field
                        if (isEditing && editableMedication["medType"] == "Eye Medication")
                          _buildDropdownField("Application Site", "applicationSite", ["Left", "Right", "Both"])
                        else if (!isEditing && editableMedication["medType"] == "Eye Medication")
